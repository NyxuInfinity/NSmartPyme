...
interface TablaCategoriesProps {
  categorias: Categoria[];
  onEditar: (categoria: Categoria) => void;
  onEliminar: (id: number) => void;
}

const TablaCategorias = ({ categorias, onEditar, onEliminar }: TablaCategoriesProps) => {
  return (
      ...
      <table className="w-full">
        ...
        <tbody className="divide-y">
          {categorias.map((categoria) => (
            <tr key={categoria.id_categoria} className="hover:bg-gray-50 transition">
              <td className="px-6 py-4 text-sm font-medium text-gray-900">{categoria.nombre}</td>
              <td className="px-6 py-4 text-sm text-gray-600">
                {categoria.descripcion || <span className="text-gray-400">-</span>}
              </td>
